			...
			if (this.canCreateRecord) {
				try {
					...
					const bill =
						this.type === 'income'
							? this.info.bill +
							  (this.amount / 100) * (100 - this.info.delta)
							: this.info.bill - this.amount;

					const deltaAmount =
						this.type === 'income'
							? this.info.deltaAmount +
							  (this.amount / 100) * this.info.delta
							: this.info.deltaAmount;
					...
					this.$message(
						`Запись успешно создана. На вашем счету ${bill}. Ваша Дельта ${deltaAmount}.`
					);
					this.$v.$reset();
					this.amount = '';
					this.description = '';
				...
				this.$message(
					`Недостаточно средств на счету (${this.amount -
						this.info.bill})`
				);
			}
		}
